...
function clearStatusAlert()
    if dragAlertTimer then
        dragAlertTimer:stop()
        dragAlertTimer = nil
    end

    if not dragAlertDrawings then return end

    for _, drawing in ipairs(dragAlertDrawings) do
        drawing:delete()
    end
...
function showStatusAlert(message)
    ...
    local textSize = dragAlertStyle.textSize or 16
    local textFont = dragAlertStyle.textFont or ".AppleSystemUIFont"
    local textFrame = hs.drawing.getTextDrawingSize(message, { font = textFont, size = textSize })
    local strokeWidth = dragAlertStyle.strokeWidth or 1
    local paddingX = dragAlertStyle.paddingX or 14
    local paddingY = dragAlertStyle.paddingY or 10
    local marginX = dragAlertStyle.marginX or 20
    local marginY = dragAlertStyle.marginY or 20
    local boxWidth = math.ceil(textFrame.w) + paddingX * 2 + strokeWidth * 2
    local boxHeight = math.ceil(textFrame.h) + paddingY * 2 + strokeWidth * 2
    local boxX = screenFrame.x + screenFrame.w - boxWidth - marginX
    local boxY = screenFrame.y + screenFrame.h - boxHeight - marginY

    local background = hs.drawing.rectangle({
        x = boxX,
        y = boxY,
        w = boxWidth,
        h = boxHeight
        ...
        :setStrokeColor(dragAlertStyle.strokeColor)
        :setFill(true)
        :setFillColor(dragAlertStyle.fillColor)
        :setRoundedRectRadii(dragAlertStyle.radius, dragAlertStyle.radius)

    local text = hs.drawing.text({
        x = boxX + paddingX,
        y = boxY + paddingY,
        w = boxWidth - paddingX * 2,
        h = boxHeight - paddingY * 2
    }, message)
        :setTextFont(textFont)
        :setTextSize(textSize)
        :setTextColor(dragAlertStyle.textColor)
        :orderAbove(background)

    dragAlertDrawings = { background, text }

    background:show()
    text:show()

    dragAlertTimer = hs.timer.doAfter(dragAlertStyle.duration or 0.2, function()
        clearStatusAlert()
    end)
end
...
-- Finder Cmd+V clipboard-image paste
require("finder_paste")
